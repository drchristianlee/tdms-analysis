...
%for quantitiative analysis kinda0
...
for i = 1:A 
    for j = 2
        if Lick_log(i,1)<51 && Lick_log(i,j)>=3.15 && Lick_log(i,j)<=5.15
            counter1 = counter1 + 1;            
        end
    end
end
for i = 1:A
    for j = 2
        if Lick_log(i,1)>50 && Lick_log(i,1)<101 && Lick_log(i,j)>=3.15 && Lick_log(i,j)<=5.15
            counter2 = counter2 + 1;
        end
    end
end

avg_num_below = counter1/50;
avg_num_above = counter2/50;

save('base_licks.mat' , 'avg_num_below');
save('led_licks.mat' , 'avg_num_above');

% test1=transpose(Lick_log(1:657,2));
% counter3=0;
% for i = 1:657
%     if test1(i)<3.15 || test1(i)>5.15
%         counter3=counter3+1;
%     end
% end
...
% sums = 0;
%  for i = 1:100
%      place_holder = number_holder(i);
%      sums = sum(number_holder(1:i));
%      for j = 1:place_holder
%         if i==1
%             if Lick_log(j,2)>=3.15 && Lick_log(j,2)<=5.15
%             elseif Lick_log(sums-number_holder(i)+j,2)<3.15 || Lick_log(sums-number_holder(i)+j,2)>5.15
%                 number_holder(i) = number_holder(i)-1;
%             end
%         else
%             if Lick_log(sums-number_holder(i)+j,2)>=3.15 && Lick_log(sums-number_holder(i)+j,2)<=5.15
%             elseif Lick_log(sums-number_holder(i)+j,2)<3.15 || Lick_log(sums-number_holder(i)+j,2)>5.15
%                 number_holder(i) = number_holder(i)-1;
%             end
%         end
...
% 
% avg_rate_below_baseline = sum(number_holder(1:50))/50;
% avg_rate_above_baseline = sum(number_holder(51:100))/50;
